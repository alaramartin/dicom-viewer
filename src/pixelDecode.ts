// All pixel-decoding logic: parsing pixel data, the per-pixel color/
// windowing loops, and the JPEG/RLE/WASM codecs for compressed transfer
// syntaxes. Deliberately split out of getImage.ts (which keeps the much
// lighter metadata-table logic) so that only this heavy module — and its
// jpeg-js/jpeg-lossless-decoder-js/pngjs/WASM-codec dependencies — gets
// bundled into dist/imageWorker.js. extension.ts never imports this file
// directly; it only talks to it through src/imageDecodeWorker.ts, which
// runs it in a worker_thread so a large CT/MR file's decode can't freeze
// the extension host's main thread (and, as a bundling side effect, keeps
// dist/extension.js from also carrying this weight).
...
// instantiated (async) once per worker lifetime, then reused, since each
// instantiation compiles a WASM module.
...
export async function convertDicomToBase64(filepath: string, frameIndex: number = 0): Promise<string> {
    try {
        ...
        if (!pixelData || !rows || !cols) {
            // not a decode failure — this file genuinely has no image (no
            // PixelData/Rows/Columns at all), e.g. a metadata-only OT/SR-type
            // object. Distinct sentinel from "compressed" so the extension
            // host can show an accurate message instead of the generic
            // decode-error page, and — importantly — can still treat the
            // file as fully editable (there's no compressed-transfer-syntax
            // reason to block edits here).
            return "no-image";
        }
